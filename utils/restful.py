...
#根据不同的状态码返回不同的内容 这些方法定一个统一的返回模板
#以成功为例子
...
def params_error(message='',data=None): #这个message等于空 是为了在不同的视图函数中 手动指定返回被容
    return result(code=HttpCode.paramserror,message=message,data=data)

def unauth(message='',data=None): #这个message等于空 是为了在不同的视图函数中 手动指定返回被容
    return result(code=HttpCode.unauth,message=message,data=data)
def method_error(message='',data=None): #这个message等于空 是为了在不同的视图函数中 手动指定返回被容
    return result(code=HttpCode.methoderror,message=message,data=data)

def server_error(message='',data=None): #这个message等于空 是为了在不同的视图函数中 手动指定返回被容
